...
class OVS extends PDO {
    ...
    function connection($dbname) {
        if ($dbname == 'CDR') {
            $CDR_DSN = 'mysql:dbname=switchcdr;host=localhost';
            $CDR_DSN_LOGIN = 'ovswitch';
            $CDR_DSN_PASSWORD = 'ovswitch123';
            try {
            ...
            $SWITCH_DSN = 'mysql:dbname=switch;host=localhost';
            $SWITCH_DSN_LOGIN = 'ovswitch';
            $SWITCH_DSN_PASSWORD = 'ovswitch123';
            try {
    ...
    function main($REQUEST) {
        ...
        if ($this->status == 'FAIL') {
            ...
            $this->writelog("SAM: Before call to PSTN_routing");
            $this->PSTN_routing();
        }
        if ($this->status == 'FAIL') {
            $responce = $this->PSTN_route_dialplan_xml();
        ...
        $this->writelog("SAM: Before call to PSTN_route_dialplan_xml");
        $responce = $this->PSTN_route_dialplan_xml();
        RETURN $responce;
    }
    ...
    function DID_Call() {
        ...
        $APIDOAMIN = "https://45.126.208.167/";
...
//  $this->callernumber = trim($this->reseller_callerid['add_string'] . trim($this->callernumber));
    ...
    function PSTN_route_routing_lb($data) {
        ...
        foreach ($routing_data as $route) {
            $this->writelog('SAM: function PSTN_route_routing_lb -> ' . $route['carrier_id']);
            $allow = 0;
    ...
    function PSTN_routing() {
        ...
        if (count($fs) > 0) {
            $this->writelog('Result are ok: count(fs) > 0');
        } else {
            $this->writelog('count(fs) = 0');
        ...
        if (count($fs) > 0) {
            ...
            foreach ($routelist as $data) {
                $this->writelog("SAM data: " . $data);
                $this->writelog("SAM data[priority] " . $data['priority']);
                if ($data['priority'] == 0)
                ...
                if ($route_p == 0) {
                    ...
                    if (count($route0) > 0) {
                        ...
                        if (count($routelist) > 0) {
                            ...
                            $this->writelog('SAM: route_p == 0 -> Before function PSTN_route_xml_builder');
                    ...
                    if (count($route1) > 0) {
                        ...
                        if (count($routelist) > 0) {
                            $this->orderBy($routelist, 'priority ASC');
                            $this->writelog('SAM: route_p == 1 -> Before function PSTN_route_xml_builder');
                            $this->PSTN_route_xml_builder($routelist);
                        }
                    }
                } elseif ($route_p == 2) {
                    ...
                    $this->orderBy($routelist, 'priority ASC');
                    $this->writelog('SAM: route_p == 2 -> Before function PSTN_route_xml_builder');
                    $this->PSTN_route_xml_builder($routelist);
                } elseif ($route_p == 3) {
                    $routelist = Array();
                    if (count($route3) > 0) {
        ...
        foreach ($routelist as $route1) {
            if ($route1['route_status'] == '1') {
                ...
                if (count($rs2) > 0) {
                    ...
                    $this->writelog('SAM function PSTN_route_xml_builder: ' . $str);
    ...
    function PSTN_route($data) {
        ...
        foreach ($data as $route2) {
            ...
            //SAM 24-06-2024 troncal_salida: $route2['ipaddress']
            $this->writelog('SAM: troncal salida: ' . $route2['ipaddress'] . " and " . $route2['rates_status'] . " and " . $route2['carrier_status'] . " and " . $route2['ip_status']);
            if ($this->status == 'OK') {
                if (strlen($route2['ipaddress']) > 0 and $route2['rates_status'] == '1' and $route2['carrier_status'] == '1' and $route2['ip_status'] == '1') {
                    $this->writelog('SAM: build Gateway_XML'); 
                    $this->out_dialplan = '1';
                    $this->writelog("SAM check destination_number: $this->destination_number");
                    $dialnumber = $this->destination_number;
                    $destination_number = $this->route_change_prefix($dialnumber, $route2);
                    $this->writelog("SAM destination_number after route_change_prefix: $destination_number");
                    ...
                    if ($gateway_type == 'CUSTOMER') {
                        ...
                        $this->Gateway_XML .= "\n   <action application=\"export\" data=\"sip_contact_user=" . $cli_from . "\"/>";
                        $this->writelog('SAM: PSTN route destination ' . $destination_number); 
                        //$this->Gateway_XML .= "\n <action application=\"bridge\" data=\"{sip_invite_domain=" . $lb . "}sofia/internal/" . $destination_number . "@" . $lb . "\"/>";
                        $this->Gateway_XML .= "\n <action application=\"bridge\" data=\"{sip_invite_domain=" . $lb . "}sofia/internal/" . $destination_number . "@" . $route2['ipaddress'] . "\"/>";
                    }
                    $cli_from = '';
                } else {
    ...
    function PSTN_route_dialplan_xml() {
        ...
        if (strlen($this->Gateway_XML) > 0) {
            $this->writelog('SAM - PSTN_route_dialplan_xml - Gateway_XML ' . $this->Gateway_XML);
            $responce .= $this->Gateway_XML;
            $this->livecalls();
        } else {
            $responce .= $this->Gateway_XML1;
    ...
    function cdrinsert() {
        ...
        $this->writelog("S.A.M. Billing Parameters: ");
        $this->writelog("Leg: ".$this->leg);
        $this->writelog("Calltype: ".$this->calltype);
        
        if ($this->leg == 'A' and $this->calltype != 'INCOMING') {
            $this->writelog("######### OutBound A leg #########");
            $this->writelog("######### OutBound A leg #########");
            $this->writelog("SAM: switch_issue:" . $this->switch_issue);
            $this->writelog("SAM: billsec:" . $this->cdr_variable['billsec']);
            $this->writelog("SAM: gateways:" . $gateways);
            $this->writelog("SAM: users:" . $users);
    ...
    function cdr_process($data) {
        $this->writelog("Line 4904: Inside function cdr_process");
        $dbdate = date('Ym');
        ...
        if ($this->switch_issue == '0') {
            ...
            //SAM 25-06-2024
            $reseller1_duartion = 0;
            $reseller2_duartion = 0;
            $reseller3_duartion = 0;            
            ...
            //Modified by JRBL: 28/02/2024
            $carrier_tax1_cost = '0.000';
            $carrier_tax2_cost = '0.000';
            $carrier_tax3_cost = '0.000';
            $carrier_callcost_total = '0.000';
            $customer_tax1_cost = '0.000';
            $customer_tax2_cost = '0.000';
            $customer_tax3_cost = '0.000';
            $customer_callcost_total = '0.000';
            $reseller1_tax1_cost = '0.000';
            $reseller1_tax2_cost = '0.000';
            $reseller1_tax3_cost = '0.000';
            $reseller1_callcost_total = '0.000';
            $reseller1_cost = '0.000';
            $reseller2_tax1_cost = '0.000';
            $reseller2_tax2_cost = '0.000';
            $reseller2_tax3_cost = '0.000';
            $reseller2_callcost_total = '0.000';
            $reseller2_cost = '0.000';
            $reseller3_tax1_cost = '0.000';
            $reseller3_tax2_cost = '0.000';
            $reseller3_tax3_cost = '0.000';
            $reseller3_callcost_total = '0.000';
            $reseller3_cost = '0.000';
        ...
        $this->writelog("S.A.M. Balance Update: ");
        $this->writelog($this->userdata['account_id']." cdr_variable['billsec']: ".$this->cdr_variable['billsec']);
        $this->writelog($this->userdata['account_id']." switch_balance: ".$this->switch_balance);

        if ($this->switch_issue == '0' and addslashes($this->cdr_variable['billsec']) > 0 and $this->switch_balance == '1') {
            $this->writelog("S.A.M. Executing Balance Update");
