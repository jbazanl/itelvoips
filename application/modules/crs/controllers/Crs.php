...
class Crs extends MY_Controller {
    ...
    function editvoip($id = '-1', $active_tab = 1) {
        ...
        $logged_account_id = get_logged_account_id();
        $logged_user_group = get_logged_user_type();
        //print_r($logged_account_id);
        //print_r($logged_user_group);
        ...
        if (check_logged_user_group(array('RESELLER'))) {
            ...
            //SAM 24-06-2024 add 2 // and 2 $$
            //$$ //$data['tariff_options'] = $this->crsvoip_mod->get_tariffs($currency_id, $logged_user_group, 'CUSTOMER');
            //$$ if ($account_type == 'RESELLER')
            //$$    $data['tariff_options'] = $this->crsvoip_mod->get_tariffs($currency_id, $logged_account_id, 'CARRIER');
            //$$else
            //$$    $data['tariff_options'] = $this->crsvoip_mod->get_tariffs($currency_id, $logged_account_id, 'CUSTOMER');         
            $data['tariff_options'] = $this->crsvoip_mod->get_tariffs($currency_id, $logged_user_group, 'CUSTOMER');
        ...
        if (check_logged_user_group(array('RESELLER'))) {
            ...
            $route_data = $this->route_mod->get_data('dialplan_name', '', '', array(), array());
